# LumynaX Coder Qwen2.5 14B Instruct GGUF

> 🇳🇿 **Made in Aotearoa New Zealand** by **AbteeX AI Labs** — part of the **LumynaX** sovereign-AI release family.

[🤗 Weights & model card on Hugging Face](https://huggingface.co/AbteeXAILab/lumynax-coder-qwen25-14b-instruct-gguf) · [abteex.com](https://abteex.com) · [lumynax.com](https://lumynax.com) · [Monorepo](https://github.com/Aimaghsoodi/lumynax-release)

---

This GitHub repo mirrors the **runtime scaffolding** (quickstart, requirements, Modelfile, Space, docs). The model **weights** are published on Hugging Face at `AbteeXAILab/lumynax-coder-qwen25-14b-instruct-gguf` — they exceed GitHub's per-file limit.

```bash
hf download AbteeXAILab/lumynax-coder-qwen25-14b-instruct-gguf --local-dir lumynax-coder-qwen25-14b-instruct-gguf
cd lumynax-coder-qwen25-14b-instruct-gguf && pip install -r requirements.txt && python quickstart.py --interactive
# Or:  pip install lumynax && lumynax run lumynax-coder-qwen25-14b-instruct-gguf -i
```
...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Coder Qwen2.5 14B Instruct GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Coder Qwen2.5 14B Instruct GGUF</h1>
...
```bash
hf download AbteeXAILab/lumynax-coder-qwen25-14b-instruct-gguf --local-dir lumynax-coder-qwen25-14b-instruct-gguf
cd lumynax-coder-qwen25-14b-instruct-gguf
...
llm = Llama(model_path="qwen2.5-coder-14b-instruct-q4_k_m.gguf", n_ctx=32768, n_threads=8, verbose=False)
...
```bash
llama-cli -m "qwen2.5-coder-14b-instruct-q4_k_m.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-coder-qwen25-14b-instruct-gguf -f ollama/Modelfile
ollama run lumynax-coder-qwen25-14b-instruct-gguf
```

**Verify integrity before launch:**

```bash
sha256sum "qwen2.5-coder-14b-instruct-q4_k_m.gguf"
cat checksums.sha256
```

```powershell
Get-FileHash -Algorithm SHA256 "qwen2.5-coder-14b-instruct-q4_k_m.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Coder Qwen2.5 14B Instruct GGUF` |
| Repository | `AbteeXAILab/lumynax-coder-qwen25-14b-instruct-gguf` |
| Family | `qwen` |
...
| Field | Value |
| --- | --- |
| Primary | `qwen2.5-coder-14b-instruct-q4_k_m.gguf` |
| Weight size | `8.37 GB` |
| Parameters | `14.0B` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `Qwen/Qwen2.5-Coder-14B-Instruct` |
...
| Primary artifact | `qwen2.5-coder-14b-instruct-q4_k_m.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
```text
lumynax-coder-qwen25-14b-instruct-gguf/
├── README.md                       # this card
...
└── qwen2.5-coder-14b-instruct-q4_k_m.gguf# primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Coder Qwen2.5 14B Instruct GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
